...
if run_sbuild; then
	...
	build_logs=("${build_dir}/${source_pkg}_${full_version}"*.build "${build_dir}"/**/"${source_pkg}_${full_version}"*.build)
	if [[ ${#build_logs[@]} -gt 0 ]]; then
		printf '%s\n' "sbuild failed; showing tail of ${build_logs[-1]}" >&2
		tail -n 200 "${build_logs[-1]}" >&2 || true
	fi
	exit "$status"
fi
...
for file in \
	"${parent_dir}/${source_pkg}_${upstream_version}.orig.tar."* \
	"${parent_dir}/${source_pkg}_${upstream_version}.orig-"*.tar.*; do
	if [[ -f "$file" ]]; then
