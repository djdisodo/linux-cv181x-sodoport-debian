...
git show-ref --verify --quiet "$source_ref" || die "missing source branch: ${remote}/${source_branch}"
git worktree add --force -B "$generated_branch" "$workspace" "$source_ref" >/dev/null
...
main_orig=$(find "$download_dir" -maxdepth 1 -type f -name "${source_pkg}_${upstream_version}.orig.tar.*" | sort -V | tail -n1 || true)
[[ -f "$main_orig" ]] || die "missing main orig tarball for ${source_pkg} ${upstream_version}"
